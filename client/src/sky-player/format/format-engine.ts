...
export class FormatEngine extends Engine<IFormatProvider> {
  ...
  private formatMedia(data: ArrayBuffer): void {
    ...
    try {
      ...
      if (error instanceof UnknowFormatMessage) {
        this._isUnknowFormat = true;
        this.send(error);
      } else if (error instanceof Error) {
        Logging.Error(FormatEngine.name, error.message);
      } else {
        Logging.Error(FormatEngine.name, `${error}`);
      }
    }
  }
  ...
  private monitorDemuxer(): void {
    if (!this._demuxer) return;
    this._demuxer.onAudioPacketCompleted =
      this.onAudioPacketCompleted.bind(this);
    this._demuxer.onSubtitlePacketCompleted =
      this.onSubtitlePacketCompleted.bind(this);
    this._demuxer.onVideoPacketCompleted =
      this.onVideoPacketCompleted.bind(this);
  }
